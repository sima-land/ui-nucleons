...
export const LINK_COLORS = ['blue', 'white', 'black', 'red', 'gray', 'dark-blue'];
 ...
 * @param {boolean} disableHoverEffect Не реагировать при наведении.
 * @param {boolean} inline Свойство inline.
 * @param {boolean} relative Относительное позиционирование.
 * @return {string} Строка с классами для ссылки/псевдо-ссылки.
 */
export const createLinkStyle = ({ className = '', disableHoverEffect = false, inline = false, relative = false }) =>
  cx(className, {
    link: true,
...
export const createExternalStyle = color => {
  const iconColor = EXTERNAL_COLORS.includes(color) ? color : 'blue';
